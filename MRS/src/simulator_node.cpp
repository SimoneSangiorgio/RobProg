...
#include <nav_msgs/Odometry.h> // AGGIUNTO: Header per il messaggio di odometria
#include <geometry_msgs/TransformStamped.h>

#include <tf2/LinearMath/Quaternion.h>
#include <tf2_ros/transform_broadcaster.h> // AGGIUNTO: Header corretto per il broadcaster
...
// ===================================================================
// IMPLEMENTAZIONE METODI SIMULATOR
// ===================================================================
...
Simulator::Simulator(ros::NodeHandle& nh) : nh_(nh) {
    ...
    for (int i = 0; i < robots_.size(); ++i) {
        ...
        // AGGIUNTO: Creazione del publisher per l'odometria
        ...
        // Creazione dei publisher per i sensori
...
void Simulator::loadParameters() {
    ...
    for (int i=0; i<robots_list.size(); ++i) {
        ...
        // AGGIUNTO: Definizione del frame_id per l'odometria
...
void Simulator::loadMap() {
    // Il tuo codice per caricare la mappa è corretto, lo lascio invariato
...
// MODIFICATO: La funzione ora pubblica sia i TF che i messaggi di odometria
void Simulator::publishState(const ros::Time& now) {
    std::vector<geometry_msgs::TransformStamped> transforms;
    for (auto& robot : robots_) {
        // --- Creazione del quaternione (comune a TF e Odom) ---
        tf2::Quaternion q;
        q.setRPY(0, 0, robot.theta);

        // --- 1. Preparazione della trasformazione TF odom -> base_link ---
        geometry_msgs::TransformStamped odom_transform;
        odom_transform.header.stamp = now;
        // MODIFICATO: Il genitore è il frame odom, non 'map'
        ...
        // --- 2. Preparazione delle trasformazioni TF statiche (base_link -> sensori) ---
        // Nota: Sarebbe meglio pubblicarle una sola volta con static_transform_publisher
        ...
        // --- 3. Pubblicazione del messaggio di Odometria ---
    ...
    // Invia tutte le trasformazioni in una volta
    tf_broadcaster_.sendTransform(transforms);
}


void Simulator::simulateAndPublishLaser(RobotState& robot, const ros::Time& now) {
    // Il tuo codice per il laser è corretto, lo lascio invariato
...
void Simulator::run() {
    ...
    while (ros::ok()) {
        ...
        update(dt);
        // MODIFICATO: Nome della funzione cambiato per chiarezza
        publishState(now); 
        for (auto& robot : robots_) {
